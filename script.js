...
if (tbody) {
tbody.innerHTML = tableData.map((t, i) => `<tr style="${i < 2 ? 'background: rgba(16, 185, 129, 0.2);' : ''}"><td class="team-name" style="text-align: left;"><div style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem;"><img src="${getFlagURL(t.name)}" class="group-flag" alt="${t.name}"><span title="${t.name}" style="white-space: nowrap;">${teamCodes[t.name]}</span></div></td><td>${t.j > 0 ? t.p : '_'}</td><td>${t.j > 0 ? t.j : '_'}</td><td>${t.j > 0 ? t.v : '_'}</td> <td>${t.j > 0 ? t.e : '_'}</td><td>${t.j > 0 ? t.d : '_'}</td><td>${t.j > 0 ? (t.sg > 0 ? '+' + t.sg : t.sg) : '_'}</td> </tr>`).join('');
}
}
}
...
// Critérios oficiais FIFA: Pontos > Saldo de Gols > Gols Pró > Vitórias > Ranking FIFA
thirdPlaces.sort((a, b) => {
    if (b.p !== a.p) return b.p - a.p;
    if (b.sg !== a.sg) return b.sg - a.sg;
    if (b.gp !== a.gp) return b.gp - a.gp;
    if (b.v !== a.v) return b.v - a.v;
    const ratingA = teamRatings[a.name] || 50;
    const ratingB = teamRatings[b.name] || 50;
